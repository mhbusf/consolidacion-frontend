...
export class ReportesCafeConJesusComponent implements OnInit {
  ...
  private readonly initial = crearFiltrosCafePredeterminados(); private request?: Subscription; private requestedPage = 0;
  ...
  private cargar(page: number, size: number): void {
    ...
    this.request = this.service.consultar(query).pipe(takeUntilDestroyed(this.destroyRef), finalize(() => this.loadingState.set(false))).subscribe({ next: value => this.pageState.set({ ...value, content: [...value.content] }), error: error => this.errorState.set(errorMessage(error)) });
  }
